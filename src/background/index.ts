...
chrome.runtime.onInstalled.addListener(() => {
  console.log(
    '🚀 ~ chrome.runtime.onInstalled.addListener ~ run at:',
    dayFormat(),
  );
  ...
  if (isDevelopment)
    menuItems.push({
      id: 'clearBackgroundConsoleLog',
      title: '清理 background 日志',
      contexts: ['all'],
    });
...
chrome.windows.onCreated.addListener(window => {
  ...
  if (
    ...
    window.height &&
    window.left &&
    window.width / window.left < 0.2 &&
    window.id
  ) {
    chrome.windows.remove(window.id);
    console.log('关闭窗口! id: ', window.id);
  }
});
